...
- ScaleTools.js: 音名表示と、設定したスケールに基づいて選択ノートを補正・移動するツール
...
### ScaleTools.js
- スクリプトパネル用スケールツールスクリプト
    - 此岸さくら氏作スクリプト（ScaleManager.js）を参考に作成。
    - 設定したスケールに基づいて、選択ノートを補正・移動するためのツール。
        - スケールから外れている音を修正したり、ハモリ用に 3 度上・5 度上などへ移動可能。
        - showAdvancedIntervals をtrue にすると8度・4度・6度・7度へも対応。
    - スケール設定はグループ別にプロジェクトファイルに保存されます。
    - 選択ノートの音名を C4 / mid2C などの形式で表示。
        - ノートが複数選択されている場合は、最低音～最高音の音域を表示。
